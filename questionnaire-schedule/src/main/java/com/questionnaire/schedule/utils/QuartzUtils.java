package com.questionnaire.schedule.utils;

import com.questionnaire.schedule.entity.JobInfo;
import org.quartz.*;
...
public class QuartzUtils {
     ...
     */
    public static void createScheduleJob(Scheduler scheduler, JobInfo jobInfo) {
        try {
            //获取到定时任务的执行类  必须是类的绝对路径名称
            //定时任务类需要是job类的具体实现 QuartzJobBean是job的抽象类。
            Class<? extends Job> jobClass = (Class<? extends Job>) Class.forName(jobInfo.getJobClass());
            // 构建定时任务信息
            JobDetail jobDetail = JobBuilder.newJob(jobClass).withIdentity(jobInfo.getJobName()).build();
            // 设置定时任务执行方式
            CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule(jobInfo.getCronExpression());
            // 构建触发器trigger
            CronTrigger trigger = TriggerBuilder.newTrigger().withIdentity(jobInfo.getJobName()).withSchedule(scheduleBuilder).build();
            // 创建定时任务
     ...
     * @throws SchedulerException
     */
    public static void updateScheduleJob(Scheduler scheduler, JobInfo jobInfo)  {
        try {
            //获取到对应任务的触发器
